...
# ── Oracle (best reward per task) ─────────────────────────────────────────
def build_oracle(records):
    best = defaultdict(lambda: -999.0)
    for r in records:
        tid = r["task_id"]
        best[tid] = max(best[tid], r["reward"])
    return dict(best)
...
def run_multi_seed(policy_cls, policy_kwargs, tasks, lookup, oracle):
    ...
    for seed in SEEDS:
        kwargs = {**policy_kwargs, "seed": seed}
        policy = policy_cls(**kwargs)

        r = run_policy(policy, tasks, lookup, oracle)
...
def plot_regret_curves(results, out_dir):
    ...
    ax.set_title("Regret Curves (Real Data)")
    ax.legend()
    ax.grid(True)
        ...
        for i, seed in enumerate(SEEDS):
            for ep, val in enumerate(arr[i]):
                rows.append({
                    ...
                    "seed": seed,
                    "episode": ep,
                    "cumulative_regret": round(float(val), 6),
                })
...
def main():
    ...
    linucb = run_multi_seed(
        LinUCBBandit,
        {"feature_dim": FEATURE_DIM, "alpha": 1.0},
        tasks, lookup, oracle
    )

    egreedy = run_multi_seed(
        EpsilonGreedyBandit,
        {"epsilon": 0.15},
        tasks, lookup, oracle
    )
